...
class TernarySalt:
	...
	def mount(self, nomenclature = Nomenclature.IUPAC):
		...
		if nomenclature == Nomenclature.TRADITIONAL:
			...
			return Compound.getPrefixNo1(self.oxygen.quantity) + "osso" + self.nonmetal.prefix2 + 'ato' + Compound.getRomanParenthesis(oxNonMet) + ' di ' + self.metal.name + Compound.getRomanParenthesis(oxMet, self.metal)	# it is important not to pass noRedundancy for non metals (their oxidation has been changed at the beginning)
